    ...
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      <DialogContent className="sm:max-w-md flex flex-col items-center justify-center p-8 bg-zinc-950/90 backdrop-blur-xl border-zinc-800 text-white rounded-2xl shadow-2xl">
        ...
        <div
          ...
          className={`relative w-64 h-64 rounded-full overflow-hidden border-[6px] transition-all duration-300 shadow-2xl cursor-pointer hover:scale-105 active:scale-95 flex items-center justify-center bg-zinc-900 ${isListening ? 'border-green-500 shadow-[0_0_30px_rgba(74,222,128,0.4)]' :
            isThinking ? 'border-amber-500 shadow-[0_0_30px_rgba(245,158,11,0.4)]' :
        ...
        >
          ...
          {isConnecting && (
            <div className="absolute inset-0 bg-black/40 z-30 flex items-center justify-center">
              <span className="animate-pulse font-semibold tracking-wider text-xs">BOOTING AI</span>
            </div>
          )}
        </div>

        {/* Dynamic Status Text */}
        <p className={`mt-6 text-sm font-medium tracking-wide transition-all duration-300 ${isListening ? 'text-green-400' : isThinking ? 'text-amber-400' : 'text-zinc-500'
          }`}>
          {isConnecting ? "Connecting to Dvir's Neural Core..." :
            isThinking ? "Consulting internal Knowledge Base..." :
              isListening ? "Listening intently to your voice..." :
                isConnected ? "Online. Ask me anything!" : "System Offline"}
        </p>
        ...
        <div className="flex flex-wrap items-center justify-center gap-3 mt-6 w-full">
          <Button
            ...
            className={`flex-1 min-w-[130px] rounded-full font-bold shadow-lg transition-all ${isListening ? 'bg-red-500 hover:bg-red-600' : 'bg-emerald-600 hover:bg-emerald-500 text-white'
              }`}
          >
