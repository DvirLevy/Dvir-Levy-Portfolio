...
export class LambdaService {
  ...
  static async DataAnalytics(data) {
    try {
      ...
      const res = await fetch(
        `${import.meta.env.VITE_DATA_ANALYTICS_URL}/${data.awsRoute}`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
  ...
  static async regressionTest(repo: string, email?: string) {
    try {
      const bodyPayload = email ? { repo, email } : { repo };
      const res = await fetch(
        `${import.meta.env.VITE_AUTHOMATION_URL_TRIGER}`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
